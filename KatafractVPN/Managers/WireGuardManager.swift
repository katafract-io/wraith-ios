...
// com.katafract.vpn.tunnel and must be included in the Xcode project.
...
final class WireGuardManager: ObservableObject {
    ...
    private let tunnelBundleId = "com.katafract.vpn.tunnel"
    ...
    private func provisionPeer(pubkey: String, server: VPNServer) async throws -> ProvisionResponse {
        let label = "KatafractVPN-\(UIDevice.current.name.prefix(20))"
    ...
    private func installProfile(configText: String, server: VPNServer) async throws {
        ...
        mgr.localizedDescription  = "Katafract VPN — \(server.cityName)"
        mgr.isEnabled             = true
