...
//   4. Calls /v1/token/validate/apple to exchange for a Katafract subscription token
...
enum KatafractProduct: String, CaseIterable {
    case armorMonthly = "com.katafract.vpn_armor_monthly"
    case armorAnnual  = "com.katafract.vpn_armor_annual"

    var displayName: String {
        switch self {
        case .armorMonthly: return "VPN Armor — Monthly"
        case .armorAnnual:  return "VPN Armor — Annual"
        }
    }
}
...
final class StoreKitManager: ObservableObject {
    ...
    private let bundleId = Bundle.main.bundleIdentifier ?? "com.katafract.vpn"
    ...
    func fetchProducts() async {
        ...
        do {
            let ids = Set(KatafractProduct.allCases.map(\.rawValue))
